...
Runner A khởi động                  Firebase RTDB              Runner B khởi động
        ...
        │  → process thoát với exit code 0 (mặc định)
```

**Quy tắc:** runner nào ghi owner sau cùng thì được chạy. Runner cũ phải tự thoát.

Ngoài ra, listener hiện phân biệt rõ 2 trường hợp khi đọc value scalar khác `STOP_RUNNER_ID`:

- `stop-<STOP_RUNNER_ID>` hoặc `stop:<STOP_RUNNER_ID>`: coi là **explicit stop token** nhắm đúng runner hiện tại.
- Bất kỳ scalar khác nào: coi là **ownership changed**.

Vì vậy, nếu bạn chỉ muốn test mất ownership, nên ghi một owner thay thế kiểu `replacement-...` để log dễ hiểu hơn, thay vì dùng tiền tố `stop-`.
...
| `STOP_REQUEST_EXIT_CODE`    | ❌       | `0`                       | Exit code dùng khi remote stop / ownership loss được phát hiện |
...
6. Process keepalive thoát bằng **exit code 0** theo mặc định để CI coi đây là stop hợp lệ. Có thể override bằng `STOP_REQUEST_EXIT_CODE`.
...
## Ghi chú về warning `The "i" variable is not set`

Nếu bạn viết `docker-compose.yml` có chuỗi dạng `${i}` bên trong command/script, Docker Compose sẽ thử **interpolate như biến môi trường của Compose** ngay từ lúc parse file. Khi env `i` không tồn tại, nó sẽ log warning kiểu:

```text
The "i" variable is not set. Defaulting to a blank string.
```

Đây là warning của **Docker Compose**, không phải của `keepalive` hay `stop-listener`. Nếu muốn giữ literal shell variable cho container, hãy escape thành `$${i}`.
